...
class Solution {
   public:
    ...
    vector<string> findAllConcatenatedWordsInADict(vector<string>& words) {
        ...
        vector<string> res;
        for (auto str : words) {
            if (!str.empty() && splitable(str, set)) {
                res.push_back(str);
            }
        }
        return res;
    }
    int splitable(const string& s, unordered_set<string>& set) {
        int n = s.size();
        vector<int> dp(n + 1, 0);
        set.erase(s);  // remove self
        dp[n] = 1;     // base case
    ...
    int isword(const string& s, unordered_set<string>& set) {
    ...
    int dfs(const string& w, int start, int cur_words) {
        int size = w.size();
        if (start == size) {
            return cur_words >= 2;
        }
        ...
        if (start > size)
            return 0;
        // search from trie root
        TrieNode* n = &trie;
        for (int i = start; i < size; i++) {
            if (n->child.find(w[i]) == n->child.end()) {
                return 0;
            }
            n = n->child[w[i]];
            if (n->isword) {
                if (dfs(w, i + 1, cur_words + 1)) {
                    return 1;
                }
            }
        }
...
int main() {
    ...
    vector<string> words, r;
    {
        ...
        r = so.findAllConcatenatedWordsInADict(words);
        printvector(r);
        r = so.findAllConcatenatedWordsInADict2(words);
        printvector(r);
    }
    {
