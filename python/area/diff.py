...
def generate_del_sql(a):
    oldLevel = a['old_risk']
    s = "delete from %s where qhdm = '%s'; %s %s-%s-%s" %(tableName,a['qhdm'],SQLCOMMENT ,a['sfmc'],a['dsmc'],get_level_label(oldLevel))
    # print(s)
...
def generate_update_sql(a):
    ...
    s = "update %s set risk = '%s' where qhdm = '%s'; %s %s-%s-%s->%s" %(tableName,a['risk'],a['qhdm'],SQLCOMMENT,a['sfmc'],a['dsmc'],old_level_lable,new_level_lable)
